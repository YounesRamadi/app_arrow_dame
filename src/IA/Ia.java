...
public class Ia {
    ...
    public int[] max(Jeu j, int ligne, int colonne, int lim, int alpha, int beta){
        ...
        int[] max={0,alpha,beta};
        int[] inter={0,alpha,beta}; //600 --> alpha | -600 --> beta
        int obl=0;
        int[][] poss;
        if(j.get_hasJumped()==1){
            ...
            if(poss!=null){
                for(int i=0; i<poss.length; i++){
                    inter=max(j,poss[i][0],poss[i][1],lim, inter[1], inter[2]);
                    if(max[0]<inter[0]){
                        max[0]=inter[0];
                    }
                }
            ...
            for (int lig = 0; lig < 7; lig++) {
                for (int col = 0; col < 9; col++){
                    if(j.get_plateau()[lig][col].get_color()==j.get_plateau()[ligne][colonne].get_color() && j.get_plateau()[lig][col] instanceof Etoile){
                        poss=j.get_possibilities(j.get_plateau()[lig][col], lig, col);
                        for(int i=0; i<poss.length; i++){
                            inter=min(j,poss[i][0],poss[i][1],lim, inter[1], inter[2]);
                            if(max[0]<inter[0])
                                max[0]=inter[0];
                        }
                    }
        ...
        if(lim==0 || j.get_nb_et_blanc()==0 || j.get_nb_et_noir()==0){
            max[0]=valuation(j, (byte)((j.get_plateau()[ligne][colonne].get_color()+1)%2));
            return max;
        }
        if(j.get_jump()==0){
                ...
                for (int col = 0; col < 9; col++){
                        ...
                        if(j.get_possible_jump()!=null){
                            for(int n=0; n<j.get_possible_jump().length; n++){
                                if ((j.get_plateau()[lig + (j.get_possible_jump()[n][0] - lig)/2][col+(j.get_possible_jump()[n][1] - col)/2].get_color()) != j.get_plateau()[lig][col].get_color()){
                                    if(obl==0)
                                        max[0]=0;
                                    inter=max(j,j.get_possible_jump()[n][0],j.get_possible_jump()[n][1],lim, inter[1], inter[2]);
                                    if(max[0]<inter[0])
                                        max[0]=inter[0];
                                    obl=1;
                                }
                    ...
                    if(j.get_possible_move()!=null && obl==0){
                        for(int n=0; n<j.get_possible_move().length; n++){
                            inter=min(j,j.get_possible_move()[n][0],j.get_possible_move()[n][0],lim-1, inter[1], inter[2]);
                            if(max[0]<inter[0])
                                max[0]=inter[0];
                        }
                    }
    ...
    public int[] min(Jeu j, int ligne, int colonne, int lim, int alpha, int beta) {
        j.move(ligne, colonne);
        int[] min={0,alpha,beta};
        int[] inter={0,alpha,beta};
        int obl=0;
        int[][] poss;
        if(j.get_hasJumped()==1){
            poss=j.get_possibilities(j.get_plateau()[ligne][colonne], ligne, colonne);
        ...
        if(lim==0 || j.get_nb_et_blanc()==0 || j.get_nb_et_noir()==0){
            min[0]=valuation(j, j.get_plateau()[ligne][colonne].get_color());
            return min;
        }
        if(j.get_jump()==0){
            j.set_hasJumped((byte) 0);
            for (int lig = 0; lig < 7; lig++) {
                for (int col = 0; col < 9; col++){
                        ...
                        if(j.get_possible_jump()!=null){
                            for(int n=0; n<j.get_possible_jump().length; n++){
                                if ((j.get_plateau()[lig + (j.get_possible_jump()[n][0] - lig)/2][col+(j.get_possible_jump()[n][1] - col)/2].get_color()) != j.get_plateau()[lig][col].get_color()){
                                    if(obl==0)
                                        min[0]=0;
                                    inter=min(j,j.get_possible_jump()[n][0],j.get_possible_jump()[n][1],lim, inter[1], inter[2]);
                                    if(min[0]>inter[0])
                                        min[0]=inter[0];
                                    obl=1;
                                }
                    ...
                    if(j.get_possible_move()!=null && obl==0){
                        for(int n=0; n<j.get_possible_move().length; n++){
                            inter=max(j,j.get_possible_move()[n][0],j.get_possible_move()[n][0],lim-1, inter[1], inter[2]);
                            if(min[0]>inter[0])
                                min[0]=inter[0];
                        }
                    }
    ...
    public int[] minMax(byte couleur, Jeu j, int lim) {
        ...
        int max=0;
        int[] inter= {0,alpha,beta};
        inter[0]=inter[1]+inter[2];                     //  !!!!!!!!!!!!!!!!!! ne sert à rien juste pour les warnings
