...
public class GameBoard {
    ...
    public int[] selection = new int[2];
    ...
    public int check_selection(int x, int y, int turn, int checkforjump) {
        if(movedPawn != null && gameboard[x][y] instanceof Fleche){
            if(x != movedPawn[0] || y != movedPawn[1]){
                System.err.println("ZEBI" + x + y + movedPawn[0] +movedPawn[1]);
                return -1;
            }
            return 0;
        }
    ...
    private int[][] get_possibilitiesStar(Pion p, int x, int y) {
        ...
        if (p.get_direction() == 0) {
            ...
            if (check_specified_pawn(x, y, x - 1, y) > 0 &&
                    check_specified_pawn(x, y, x - 2, y) == 0) {
                // pion P() -> 0
                tmp = new int[2];
                tmp[0] = x - 2;
                tmp[1] = y;
            ...
            if (check_specified_pawn(x, y, x, y + 2) == 0 &&
                    check_specified_pawn(x, y, x, y + 1) > 0) {
            ...
            if (check_specified_pawn(x, y, x - 2, y - 2) == 0 &&
                    check_specified_pawn(x, y, x - 1, y - 1) > 0) {
                tmp = new int[2];
                tmp[0] = x - 2;
                tmp[1] = y - 2;
                ar2.add(indexJump++, tmp);
            ...
            if (check_specified_pawn(x, y, x + 2, y) == 0 &&
                    check_specified_pawn(x, y, x + 1, y) > 0) {
                // pion P() -> 0
                tmp = new int[2];
                tmp[0] = x + 2;
                tmp[1] = y;
